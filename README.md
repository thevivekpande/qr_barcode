...
Keyboard capture defaults to **Automatic**, which finishes a read on Enter, Tab, or a **150 ms idle gap**, configurable from 50–2,000 ms. Strict Enter-only, Tab-only, and idle-only modes remain available. Capture pauses when the field loses focus; Escape stops it. Ordinary typing in the focused capture field is indistinguishable from a keyboard reader. This mode cannot enumerate the reader, identify its USB vendor, or send device commands.
