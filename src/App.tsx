...
function App() {
  ...
  const [settings, setSettings] = useState<CodeSettings>(defaults);
  ...
  const [charset, setCharset] = useState<'alphanumeric' | 'numeric' | 'alphabetic'>('alphanumeric');
  ...
  useEffect(() => {
    let cancelled = false;
    generateCode('https://example.com', 'qr', defaults)
      .then((code) => {
        if (!cancelled) setSingleCode(code);
      })
      .catch((err) => {
      ...
      <div className="main-shell">
        ...
        <main id="workspace">
              ...
              <div className="editor-body">
                <fieldset className="type-fieldset" disabled={running || busy}>
                  ...
                  <div className="type-switch">
                    ...
                    >
                      <Barcode size={21} /> Barcode {codeType === 'barcode' && <Check size={15} />}
                    </button>
                  </div>
                </fieldset>
                ...
                {mode === 'batch' && (
                  <div className="content-field">
                    <div className="label-row">
                      ...
                      <button className="text-button" onClick={() => setBatchInput(sampleBatch)}>
                ...
                <button
                  ...
                  disabled={busy}
                  onClick={
                    mode === 'single' ? createSingle : mode === 'batch' ? createBatch : toggleLive
                  }
                >
                  {busy ? (
            ...
            <section className="preview-card panel" aria-labelledby="preview-title">
              <div className="panel-heading">
                ...
                <span className={`preview-tag ${running ? 'is-live' : ''}`}>
                  <span />
                  {mode === 'batch' ? `${batchCodes.length} CODES` : running ? 'LIVE' : 'PREVIEW'}
                </span>
              </div>
              {mode === 'batch' ? (
          ...
          <section className="tip-banner">
            ...
            <button
              onClick={() => (mode === 'batch' ? guide.current?.showModal() : navigate('batch'))}
            >
          ...
          <footer className="page-footer">
            <span>
              <LockKeyhole size={12} /> Your content stays in your browser. Always.
            </span>
            <span>
